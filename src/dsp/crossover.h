...
//   stage B:  low_half    -> [LR4 @ f1] -> Sub      / Bass
//   stage C:  high_half   -> [LR4 @ f3] -> Mid      / High
//
...
class Crossover {
  public:
    ...
    void Init(float sample_rate) {
        ...
        SetCrossovers(settings::crossover::kSubBassHz,
                      settings::crossover::kBassMidHz,
                      settings::crossover::kMidHighHz);
        for (auto& s : stages_) {
    ...
    void SetCrossovers(float f1, float f2, float f3) {
        SetStageFreq(0, f1); // stage B (low_half  -> Sub / Bass)
        SetStageFreq(1, f2); // stage A (full      -> low_half / high_half)
        SetStageFreq(2, f3); // stage C (high_half -> Mid / High)
    }

    // Process one stereo sample. out_l/out_r ordered [Sub, Bass, Mid, High].
    void Process(float in_l, float in_r,
        ...
        // Stage B: split low_half at f1 -> Sub (band 0), Bass (band 1).
        SplitLR4(stages_[0], 0, low_half_l, out_l[0], out_l[1]);
        SplitLR4(stages_[0], 1, low_half_r, out_r[0], out_r[1]);

        // Stage C: split high_half at f3 -> Mid (band 2), High (band 3).
